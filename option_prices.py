...
if price is None:
    st.error("Could not get price."); st.stop()
exps = fetch_options(symbol)
if not exps:
    st.error("No option expirations."); st.stop()
...
with right:
    ...
    with c1:
        ...
        st.dataframe(style_atm(calls9[["contractSymbol","strike","lastPrice","bid","ask","volume","openInterest","impliedVolatility"]], atm_c), use_container_width=True)
    with c2:
        st.markdown(f"**Puts — ATM {atm_p}**")
        st.dataframe(style_atm(puts9[["contractSymbol","strike","lastPrice","bid","ask","volume","openInterest","impliedVolatility"]], atm_p), use_container_width=True)

st.markdown("---")
st.subheader("Volatility Surface")
colH, col3D = st.columns(2)
with colH:
...
with col3D:
    if st.button("Vol Surface (3D)"):
        ...
        if strikes is None: st.error("Could not build surface.")
            ...
            surf=ax.plot_surface(D,K,100*iv,linewidth=0,antialiased=True,color="#90EE90")  # uniform light green
